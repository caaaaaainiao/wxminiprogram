...
Page({
  ...
  ask: function(cardNo) {
    ...
    wx.request({
      url: app.globalData.url + '/memberCard/queryMemberCardRecharge',
      data: dataInfo,
      method: "POST",
      header: {
        ...
        if (res.data.status == 0) {
          ...
          if (res.data.data && res.data.data.data && res.data.data.data.length > 0) {
            for (let i = 0; i < res.data.data.data.length; i++) {
              res.data.data.data[i].rechargeDate = res.data.data.data[i].rechargeDate.split(" ")[0]
            }
            that.setData({
                    ...
                    wx.request({
                      ...
                      success: function (msg) {
                        if (msg.data.status == 0) {
                          wx.hideLoading();
                          if (msg.data.data && msg.data.data.data && msg.data.data.data.length > 0) {
                            for (let i = 0; i < msg.data.data.data.length; i++) {
                              msg.data.data.data[i].rechargeDate = msg.data.data.data[i].rechargeDate.split(" ")[0]
                            }
                            that.setData({
